-- Fixed vesting event generation function
-- Removes reference to non-existent exercise_price column in grants table
...
BEGIN
  ...
  -- Set default exercise price for ESOP plans (since grants table doesn't have exercise_price column)
  v_exercise_price := CASE 
    WHEN v_plan.plan_type = 'ESOP' THEN 1.0  -- Default exercise price
    ELSE NULL
  END;
  ...
  IF v_cliff_months > 0 AND v_cliff_shares > 0 THEN
    ...
    INSERT INTO vesting_events (
      ...
      CASE WHEN v_current_date <= CURRENT_DATE THEN 'due'::vesting_event_status ELSE 'pending'::vesting_event_status END,
      true
    );
