  ...
  const statCards = useMemo(() => [
    {
      ...
      if (companyData) {
        ...
        // Calculate ESOP-specific metrics
        const esopTotalShares = esopGrantsRes.data?.reduce(
          (sum, grant) => sum + Number(grant.total_shares || 0),
          0
        ) || 0;
        ...
        const ltipPoolTotals = (ltipPoolsRes.data || []).reduce(
          (acc: { total: number; used: number }, pool: any) => {
            acc.total += Number(pool.total_shares_allocated || 0);
            acc.used += Number(pool.shares_used || 0);
            return acc;
          },
          { total: 0, used: 0 }
        );

        const ltipTotalAllocated = ltipPoolTotals.total;
        const ltipTotalShares = ltipPoolTotals.used;

        // Calculate shares breakdown (granted, ungranted, vested, unvested)
        // Fetch all grants with plan information, then filter for LTIP plans
        ...
        if (grantsError) {
          console.error('Error fetching grants for breakdown:', grantsError);
        }

        // Filter for LTIP grants only (matching Equity Pool Size scope)
        const ltipGrants = (allGrantsData || []).filter(grant => {
          const planType = grant.incentive_plans?.plan_type;
          return planType && planType.startsWith('LTIP');
        });

        // Filter for valid grants (exclude only cancelled/forfeited)
        // Include: active, pending_signature, completed, and any other statuses
        const validLtipGrants = ltipGrants.filter(g => 
          g.status !== 'cancelled' && g.status !== 'forfeited'
        );
        
        // Also check if we need to include grants that might have null status
        const grantsWithNullStatus = ltipGrants.filter(g => !g.status || g.status === null);
        if (grantsWithNullStatus.length > 0) {
          console.warn('⚠️ Found grants with null status:', grantsWithNullStatus.length);
        }

        // Calculate shares by status for debugging
        const sharesByStatus = ltipGrants.reduce((acc: any, g) => {
          const status = g.status || 'unknown';
          if (!acc[status]) {
            acc[status] = { count: 0, shares: 0 };
          }
          acc[status].count += 1;
          acc[status].shares += Number(g.total_shares || 0);
          return acc;
        }, {});

        // Calculate granted shares from valid grants only (exclude cancelled/forfeited)
        const grantedSharesFromGrants = validLtipGrants.reduce(
          (sum, grant) => sum + Number(grant.total_shares || 0),
          0
        );

        // Also calculate from all grants (no filter) for comparison
        const grantedSharesAll = ltipGrants.reduce(
          (sum, grant) => sum + Number(grant.total_shares || 0),
          0
        );

        // Alternative: Calculate from incentive_plans.shares_granted for LTIP plans
        // This is the source of truth maintained by database triggers
        const { data: ltipPlansData } = await supabase
          .from('incentive_plans')
          .select('shares_granted, plan_type')
          .eq('company_id', companyId)
        ...
        // Use grantedFromPlans as the source of truth (maintained by DB triggers)
        // Fallback to grantedSharesFromGrants if plans data is not available
        const grantedShares = grantedFromPlans > 0 ? grantedFromPlans : grantedSharesFromGrants;

        console.log('📊 Shares Breakdown Debug:', {
          totalGrants: allGrantsData?.length || 0,
          ltipGrants: ltipGrants.length,
          validLtipGrants: validLtipGrants.length,
          ltipTotalAllocated,
          ltipTotalShares, // This is from ltip_pools.shares_used
          grantedSharesFromGrants, // From summing grants
          grantedSharesAll, // From all grants including cancelled/forfeited
          grantedFromPlans, // From incentive_plans.shares_granted (source of truth)
          grantedShares, // Final value used (from plans if available)
          sharesByStatus,
          differenceFromExpected: 71131 - grantedShares,
          differenceFromPlans: grantedFromPlans - grantedSharesFromGrants,
          sampleGrant: validLtipGrants[0]
        });

        // Calculate ungranted shares (total pool - granted)
        const ungrantedShares = Math.max(0, ltipTotalAllocated - grantedShares);

        // Calculate vested and unvested from active LTIP grants only
        const activeLtipGrants = validLtipGrants.filter(g => g.status === 'active');
        
        // Calculate total shares from active grants
        const activeGrantedTotal = activeLtipGrants.reduce(
          (sum, grant) => sum + Number(grant.total_shares || 0),
          0
        );
        ...
        if (activeLtipGrants.length > 0) {
          // Try to use grant fields first
          vestedShares = activeLtipGrants.reduce(
            (sum, grant) => {
              const vested = Number(grant.vested_shares || 0);
              return sum + vested;
            },
          ...
          // If vested shares are 0, try calculating from vesting events
          if (vestedShares === 0 && activeGrantedTotal > 0) {
            const grantIds = activeLtipGrants.map(g => g.id);
            const { data: vestingEventsData } = await supabase
              .from('vesting_events')
              .select('grant_id, shares_to_vest, status')
              .in('grant_id', grantIds);
        ...
        console.log('Query results:', {
          employeesCount: employeesRes.count,
          plansCount: plansRes.count,
          activeGrantsCount: activeGrantsRes.count,
          totalShares,
          closingPrice: parsedClosingPrice,
          manualFmv: parsedManualFmv,
          resolvedCurrentFmv,
          employeeSharesTotal,
          totalSharesFromShareholders,
          optionPoolBalance
        });
        ...
        setStats({
          ...
          totalShares,
          currentFMV: resolvedCurrentFmv,
          optionPoolBalance: esopTotalShares,
          totalGrants: allGrantsRes.count || 0,
          grantsAccepted: acceptedGrantsRes.count || 0,
          grantsPending: pendingGrantsRes.count || 0,
          grantsRejected: rejectedGrantsRes.count || 0,
          activeESOPEmployees: uniqueActiveESOPEmployees,
          inactiveESOPEmployees: uniqueInactiveESOPEmployees,
          equityPoolSize: ltipTotalShares,
          esopTotalAllocated,
          esopTotalShares,
          ltipTotalAllocated,
          ltipTotalShares,
