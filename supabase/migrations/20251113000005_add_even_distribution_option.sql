-- Add use_even_distribution column to grants table
ALTER TABLE grants 
ADD COLUMN IF NOT EXISTS use_even_distribution BOOLEAN DEFAULT false;

-- Update the generate_vesting_events_for_grant function to support even distribution
...
BEGIN
  ...
  -- Get even distribution preference from grant
  v_use_even_distribution := COALESCE(v_grant.use_even_distribution, false);
  ...
  IF v_use_even_distribution THEN
    ...
    IF v_cliff_months > 0 THEN
      ...
      v_cliff_shares := FLOOR(v_grant.total_shares / (v_total_periods + 1));
      v_shares_per_period := FLOOR((v_grant.total_shares - v_cliff_shares) / v_total_periods);
    ELSE
      -- No cliff: divide equally among all periods
      v_shares_per_period := FLOOR(v_grant.total_shares / v_total_periods);
      v_cliff_shares := 0;
    END IF;
  ELSE
    -- Percentage-based with floor (existing logic)
    IF v_cliff_months > 0 THEN
      v_cliff_shares := FLOOR(v_grant.total_shares * 0.25);
      v_shares_per_period := FLOOR((v_grant.total_shares - v_cliff_shares) / v_total_periods);
    ELSE
      v_cliff_shares := 0;
      v_shares_per_period := FLOOR(v_grant.total_shares / v_total_periods);
  ...
  RAISE NOTICE 'Generated % vesting events for grant % (total shares: %, allocated: %, even_distribution: %)', 
