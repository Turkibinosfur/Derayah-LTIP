-- Fix and regenerate vesting events for grant GR-20251103-000018
-- This script will:
...
BEGIN
  ...
  INTO 
    ...
    v_plan_id
  FROM grants
  WHERE grant_number = 'GR-20251103-000018';
  
  IF v_grant_id IS NULL THEN
    RAISE EXCEPTION 'Grant GR-20251103-000018 not found';
  END IF;
  ...
  IF v_milestone_count = 0 THEN
    ...
    -- Determine milestone type from template schedule type
    DECLARE
    ...
    BEGIN
      ...
      IF v_template_cliff_months > 0 THEN
        ...
        RAISE NOTICE '    Generated cliff milestone: 25%% at % months (type: %)', 
          v_template_cliff_months, v_milestone_type;
      ...
      BEGIN
        ...
        IF v_period_count > 0 THEN
          ...
          FOR i IN 1..v_period_count LOOP
            ...
            RAISE NOTICE '    Generated periodic milestone: %.2f%% at % months (type: %)', 
              v_percentage_per_period,
              v_template_cliff_months + (i * v_frequency_months),
              v_milestone_type;
            v_milestone_order := v_milestone_order + 1;
  ...
  LOOP
    ...
    RAISE NOTICE '  Event %: % shares (%.2f%%) on % (type: %)', 
      v_milestone_row.sequence_order,
      v_shares_to_vest,
